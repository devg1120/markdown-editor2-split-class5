...
export class MarkDownEditor {
    constructor(parent_id, name, base, no, editor_id, preview_id) {
        ...
        this.editor.on("click", function (e) {
            ...
            if (that.toc_index) {
                ...
                for (let i = 0; i < that.toc_index.length; i++) {
                    if (
                        that.toc_index[i].element.hasAttribute("aria-current")
                    ) {
                        that.toc_index[i].element.removeAttribute(
                            "aria-current",
                        );
                    }
                }
                ...
                for (let i = 0; i < that.toc_index.length - 1; i++) {
                    ...
                    if (a <= linenum && linenum < b) {
                        ...
                        that.toc_index[i].element.setAttribute(
                            "aria-current",
                            "page",
                        );
                        match = true;
    ...
    convert_org(id, markdown) {
        ...
        document.querySelector(this.parent_id + " " + "#" + id).innerHTML = sanitized;
        if (this.toc != null) {
...
*/
        ...
        if (this.toc != null) {
            ...
            for (let i = 0; i < tokens.length; i++) {
                if (tokens[i].type == "heading") {
                    ...
                    let linenum = this.lineOf(
                        tokens[i].raw.replace("\n", ""),
                        markdown,
                    );
    ...
    schemeChange_dark() {
        var output = document.querySelector(this.parent_id + " " + "#output" + this.no);
        output.classList.remove("light");
        output.classList.add("dark");

        var preview = document.querySelector(this.parent_id + " " + "#preview" + this.no);
        preview.classList.remove("light");
        preview.classList.add("dark");
        var preview_w = document.querySelector(this.parent_id + " " + "#preview-wrapper" + this.no);
